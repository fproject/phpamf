...
abstract class AbstractFunctionReflector
{
    ...
    public function __construct(Reflector $r, $namespace = null, $argv = array())
    {
        ...
        if ((!$r instanceof ReflectionFunction)
            ...
            throw new AmfException('Invalid reflection class');
        }
        $this->_reflection = $r;
     ...
     *
     * @param ReflectionNode $parent
     * @param int $level
     * @return void
     */
    protected function _addTree(ReflectionNode $parent, $level = 0)
    {
        ...
        foreach ($this->_sigParams[$level] as $value) {
            $node = new ReflectionNode($value, $parent);
     ...
     * @param array $paramTypes of arguments (each an array of types)
    ...
    protected function _buildSignatures($return, $returnDesc, $paramTypes, $paramDesc)
    {
        ...
        foreach ($endPoints as $node) {
            ...
            do {
                ...
                $node = $node->getParent();
            } while ($node instanceof ReflectionNode);
        ...
        foreach ($signatures as $signature) {
            ...
            foreach ($signature as $key => $type) {
                $param = new ParameterReflector($params[$key], $type, (isset($this->_paramDesc[$key]) ? $this->_paramDesc[$key] : null));
     ...
     * ReflectionFunction and parsing of DocBlock
     * @return array
     * @throws AmfException
     */
    ...
    public function __call($method, $args)
    {
        ...
        throw new AmfException('Invalid reflection method ("' .$method. '")');
    }
    ...
    {
        if (!is_string($string)) {
            throw new AmfException('Invalid description');
        }
